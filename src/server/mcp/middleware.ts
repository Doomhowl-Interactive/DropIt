...
/**
 * Authenticates an MCP client by its bearer API token.
 *
 ...
 */
export function mcpTokenAuth(tokens: ApiTokenService): RequestHandler {
  return (req: Request, res: Response, next: NextFunction) => {
    const header = req.get('authorization') ?? '';
    const [scheme, value] = header.split(' ');

    if (scheme !== 'Bearer' || !value) {
      return unauthorized(res, 'A Drop.it API token is required.');
    }
    ...
    tokens
      ...
      .then((token) => {
        if (!token) {
          return unauthorized(res, 'This API token is unknown, revoked or expired.');
        }
        req.apiToken = token;
        next();
      })
...
function unauthorized(res: Response, message: string): void {
  res.setHeader('WWW-Authenticate', 'Bearer realm="dropit"');
  res.status(401).json({
    jsonrpc: '2.0',
